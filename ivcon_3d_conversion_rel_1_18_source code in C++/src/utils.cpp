...
// Purpose:
//   Provides useful utilities for IVCON library
 ...
 */
bool char_pad(unsigned int *char_index, unsigned int *null_index, char *string,
		unsigned long STRING_MAX)
{
	int i;

	if (*char_index < 0 || *char_index >= *null_index
 ...
 */
long get_long_pos(unsigned long val, const array<unsigned long>& a,
		unsigned long n)
{
	unsigned long i = 0;
